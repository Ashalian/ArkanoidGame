...
class Tacka(object):
    ...
    def draw(self):
        pygame.draw.rect(self.game.screen, self.Colors.paddleCol, self.rect)
...
class Colors:
    ...
    paddleCol = (15,94,115)
    ballCol = (64, 71, 73)
    blueLevel1 = (197, 220, 224)
    cremeLevel2 = (241, 212, 175)
...
class Game(object):
    def __init__(self):
        ...
        while True:
            ...
            self.clock.tick(60)
            self.screen.fill(self.colors.blueLevel1)
            if self.spaceCount == 1:
                self.BallMove()
            self.draw()
            self.tick()
            self.lose()
            self.printMsg("Your score: %i" % self.points, self.colors.black, [20, 690], 18)
            self.printMsg("Your lives: %i" % self.lives, self.colors.black, [1120, 690], 18)
    ...
    def draw(self):
        ...
        if self.points < 270:
            self.Level1()
            self.printMsg("Click space to start. Use arrows to control the paddle.", self.colors.black, [325, 690], 18)

        elif self.points == 270:
            self.ball.reset(self.player.rect.centerx)
            self.points+=5
            self.screen.fill(self.colors.black)
            self.printMsg("Great job! Yor score is: %i. Get ready for second level."%self.points, self.colors.offWhite, [115, 300], 30)
            pygame.display.flip()
            pygame.time.wait(2000)
            self.spaceCount = 0
            self.ball.BallMoveBeforeSpace()

        elif self.points >= 275 and self.points < 825:
            self.Level2()

        elif self.points == 825:
            self.ball.reset(self.player.rect.centerx)
            self.points+=5
            self.screen.fill(self.colors.black)
            self.printMsg("Great job! Yor score is: %i. Get ready for third level."%self.points, self.colors.offWhite, [115, 300], 30)
            pygame.display.flip()
            pygame.time.wait(2000)
            self.spaceCount = 0
            self.ball.BallMoveBeforeSpace()

        elif self.points >= 830 and self.points < 1710:
            self.Level3()

        elif self.points == 1710:
            self.ball.reset(self.player.rect.centerx)
            self.points+=5
            self.screen.fill(self.colors.black)
            self.printMsg("Great job! Yor score is: %i. Get ready for fourth level."%self.points, self.colors.offWhite, [115, 300], 30)
            pygame.display.flip()
            pygame.time.wait(2000)
            self.spaceCount = 0
            self.ball.BallMoveBeforeSpace()

        elif self.points >= 1715 and self.points<4115:
            self.Level4()
        elif self.points == 4115:
            self.win()
        ...
        pygame.draw.rect(self.screen, self.colors.ballCol, self.ball.rect)
    ...
    def printMsg(self, msg, color, pos, size):
        self.font = pygame.font.Font('SpaceMono-Regular.ttf', size)
    ...
    def Level2(self):
        self.screen.fill(self.colors.cremeLevel2)
        for self.block in self.blocksRed:
            pygame.draw.rect(self.screen, self.colors.sunny, self.block)
    ...
    def Level4(self):
        self.screen.fill(self.colors.waterish)
    ...
    def win(self):
        self.screen.fill(self.colors.black)
        self.points+=5
        self.printMsg("You won! Your score is: %i" %self.points, self.colors.offWhite, [230, 300], 48)
        pygame.display.flip()
    ...
    def lose(self):
        if self.lives == 0:
            self.screen.fill(self.colors.black)
            self.printMsg("You lost! Your score is: %i" %self.points, self.colors.offWhite, [230, 300], 48)
            pygame.display.flip()
            pygame.time.wait(4000)
            exit(0)
